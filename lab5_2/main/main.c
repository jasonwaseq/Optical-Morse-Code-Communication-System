...
#define SAMPLE_MS 20   // sampling interval
...
void app_main(void) {
    ...
    const int threshold = 50;     // mV, midway between LED off (~10mV) and LED on (~100mV)
    const int hysteresis = 5;     // mV, prevents bouncing
    const int dot = 200;           // ms, matches your Python DOT=0.2s
